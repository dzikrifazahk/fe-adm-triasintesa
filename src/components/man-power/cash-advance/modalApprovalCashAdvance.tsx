...
import { cashAdvanceService, overtimeService } from "@/services";
...
export default function ModalApprovalCashAdvance({
  isOpen,
  title,
  onClose,
  id,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await Swal.fire({
      icon: "warning",
      text: `Apakah Anda yakin ingin ${confirmText} pengajuan pinjaman ini?`,
      showDenyButton: true,
    ...
    try {
      setIsLoading(true);
      const response = await cashAdvanceService.cashAdvanceApproval(
        id,
        payload
      );
