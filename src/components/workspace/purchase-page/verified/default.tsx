...
export default function VerifiedPageMain({ dictionary }: Props) {
  ...
  const [detailModalMode, setDetailModalMode] = useState<"details">("details");
  ...
  useEffect(() => {
    setIsLoading(false);
    getPurchases(1, pageSize, String(params?.workspace ?? ""), 2);
  }, []);
  ...
  const handleActions = async (id: string, actions: "details") => {
    if (actions === "details") {
  ...
  const handleUndoData = async (id: string) => {
    Swal.fire({
      ...
      text: "Apakah anda ingin mengembalikan data ini ke tab pengajuan?",
      showDenyButton: true,
  ...
  const handlePaymentRequest = async (id: string) => {
    Swal.fire({
      ...
      text: "Apakah anda ingin mengajukan pembayaran?",
      showDenyButton: true,
      confirmButtonText: "Ya",
      confirmButtonColor: "#493628",
      denyButtonText: "Tidak",
      ...
      if (result.isConfirmed) {
        try {
          setIsLoading(true);
          const { data, message } = await purchaseService.paymentRequest(id);
          getPurchases(
            page,
            pageSize,
            String(params.workspace),
