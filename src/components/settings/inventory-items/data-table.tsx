...
export function DataTable<TData, TValue>({
  ...
  const [pagination] = React.useState<PaginationState>({ pageIndex: 0, pageSize: 10 });
  ...
  return (
    <>
      <div className="w-full pb-4">
        <div className="grid w-full gap-3 md:gap-4 grid-cols-1 xl:[grid-template-columns:minmax(0,1fr)_auto] items-start p-1">
          ...
          <div className={controlsWrapClass}>
            <div className={searchWrapperClass}>
              <Input
                placeholder={dictionary?.search_item_placeholder ?? "-"}
                value={search}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className={`shrink-0 ${buttonWidthClass} whitespace-nowrap cursor-pointer`}>
                  {dictionary?.column_filter ?? "-"}
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align={dropdownAlign} className="max-h-[50vh] overflow-auto">
            ...
            <Button variant="outline" className={`shrink-0 ${buttonWidthClass} cursor-pointer`} onClick={() => isGetData?.()}>
              <FaArrowRotateLeft />
            </Button>
